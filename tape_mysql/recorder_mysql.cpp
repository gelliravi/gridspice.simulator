...
int recorder_mysql::init(OBJECT *parent)
{
  ...
  db = new db_access(table, myProperties, false);
...
TIMESTAMP recorder_mysql::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	for (p=this->props; p!=NULL && offset<size-33; p=p->next)
	  { 
	    offset=gl_get_value(obj->parent,GETADDR(obj->parent,p),buffer,size-1,p); /* pointer => int64 */
	    buffer[offset]='\0';      
	    std::stringstream ss;      
	    ss << buffer;      
	...
	char ts1[64]="0"; /* 0 = INIT */
	sprintf(ts1,"%" FMT_INT64 "d", t1);
